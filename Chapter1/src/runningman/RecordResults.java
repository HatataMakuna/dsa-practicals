...
public class RecordResults extends javax.swing.JFrame {
  ...
  private void jbtConfirmActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtConfirmActionPerformed
        ...
        if(name.equals("")){
            JOptionPane.showMessageDialog(null, "Please enter a valid Runner's Number", "Error", JOptionPane.ERROR_MESSAGE);
        }else{
            finisherList.add(runner);
            StringBuilder outputFinishedList = new StringBuilder();
            outputFinishedList.append("MARATHON RESULTS\n");
            for (int i = 0; i < finisherList.size(); i++) {
                outputFinishedList.append(i+1 + ". No. " +finisherList.get(i).getNumber() + "\t" + finisherList.get(i).getName() + "\n");
            }
            jtaResults.setText(outputFinishedList.toString());
        }
        StringBuilder outputFinishedList = new StringBuilder();
        outputFinishedList.append("MARATHON RESULTS\n");
  ...
  private void jtfNumberActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jtfNumberActionPerformed
        try{
            ...
            if(found == false){
                JOptionPane.showMessageDialog(null, "Runner's Number not found", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }catch(Exception ex){
    ...
    private void jtfNumberKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jtfNumberKeyReleased
        try{
            int number = Integer.parseInt(jtfNumber.getText());
            for (int i = 0; i < runnerList.size(); i++) {
                  if(number == runnerList.get(i).getNumber()){
    ...
    }//GEN-LAST:event_jtfNumberKeyReleased

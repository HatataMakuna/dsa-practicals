...
public class PostOfficeSim extends javax.swing.JFrame {
    ...
    private class CounterListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            if(!q.isEmpty()){
                int counterNo = Integer.parseInt(e.getActionCommand().replaceAll("[^\\d.]", ""));
                Customer customer = q.poll();
                customer.setCounter(counterNo);
                customer.setServeTime(Calendar.getInstance());
    ...
    private void jbtReportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtReportActionPerformed
        ...
        for (int i = 0; i < serviceList.size(); i++) {
            str += String.format("%-5s   %s %-15s\n", 
                    String.valueOf(i+1) + ".", serviceList.get(i).toString(), serviceList.get(i).getWaitingTime());
            totalWaitingTime += serviceList.get(i).getWaitingTime();
        }
        
        str += "\nTotal customer served : " + serviceList.size();
        str +=  String.format("\nAverage waiting time    : %.2f", (double)totalWaitingTime/serviceList.size());
